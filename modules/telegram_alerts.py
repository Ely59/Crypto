...
def build_filters_message() -> str:
    ...
    return "\n".join([
        ...
        "",
        "<b>Universe — CMC Screen</b>",
        f"  Universe:    Top {cfg.MOMENTUM_CMC_LIMIT} coins by 24H volume",
        f"  1H gate:     ≥{cfg.MOMENTUM_1H_MIN_MOVEMENT:.1f}% (loop stops below this)",
        f"  MCap:        ${cfg.MOMENTUM_MCAP_MIN_USD/1e6:.0f}M – ${cfg.MOMENTUM_MCAP_MAX_USD/1e9:.0f}B",
        f"  Micro-cap:   ≥${cfg.MOMENTUM_MCAP_MICRO_MIN_USD/1e6:.0f}M if Vol/MC ≥{cfg.MOMENTUM_MCAP_MICRO_VOLMC_MIN*100:.0f}%",
        f"  24H vol:     ≥${cfg.MOMENTUM_VOL_24H_MIN_USD/1e6:.0f}M  (bypassed if Vol/MC ≥{cfg.MOMENTUM_VOLMC_BYPASS_MIN*100:.0f}%)",
        f"  Dead coin:   blocked if Vol/MC &lt;{cfg.MOMENTUM_VOLMC_DEAD_MAX*100:.0f}%",
        f"  Circ supply: ≥{cfg.MOMENTUM_CIRC_SUPPLY_MIN_PCT:.0f}%",
        f"  FDV/MCap:    ≤{cfg.MOMENTUM_FDV_RATIO_MAX:.0f}×",
        f"  90d ATH:     hard block if &lt;{cfg.MOMENTUM_ATH_DIST_HARD_BLOCK:.0f}% below 90d high",
        f"  ATL buffer:  price must be ≥{cfg.MOMENTUM_ATL_BUFFER_PCT:.0f}% above ATL",
        "  Categories:  L1, L2, AI/Data, DePIN, RWA, Gaming/GameFi,",
        "               DeFi/DEX, Modular, Fan Token, Privacy",
        "",
        "<b>4H Macro Gate</b>",
        f"  EMA stack:   EMA6 &gt; EMA12 &gt; EMA20 (sep ≥{cfg.MOMENTUM_TA_H4_EMA_SEP_MIN*100:.1f}%)",
        f"  4H KDJ J:    fast-move warning at ≥{cfg.MOMENTUM_TA_H4_KDJ_J_MAX:.0f}",
        f"  Fear Mode:   activates when Fear &amp; Greed ≤{cfg.MOMENTUM_FEAR_FG_THRESHOLD} (relaxed thresholds)",
        f"  Global CD:   {cfg.MOMENTUM_GLOBAL_COOLDOWN_MIN // 60}H per coin across all signal types",
        "",
        "<b>Scoring Tiers</b>",
        f"  🟢 STRONG ENTRY ≥{cfg.MOMENTUM_TOTAL_STRONG_ENTRY}/100  |  SL {cfg.MOMENTUM_SL_PCT:.0f}%  TP1 {cfg.MOMENTUM_TP1_PCT:.0f}%  TP2 {cfg.MOMENTUM_TP2_PCT:.0f}%",
        f"  🟡 WATCH        ≥{cfg.MOMENTUM_TOTAL_WATCH}/100",
        f"  🟠 MONITOR      ≥{cfg.MOMENTUM_TOTAL_MONITOR}/100  (silent — no alert)",
        f"  Coin cooldown:  {cfg.MOMENTUM_ALERT_COOLDOWN_MIN // 60}H",
        "",
        "<b>90d ATH Distance Scoring</b>",
        f"  &gt;{cfg.MOMENTUM_ATH_DIST_L2:.0f}% below → +{cfg.MOMENTUM_ATH_DIST_L2_PTS}pts  |  {cfg.MOMENTUM_ATH_DIST_L1:.0f}-{cfg.MOMENTUM_ATH_DIST_L2:.0f}% → +{cfg.MOMENTUM_ATH_DIST_L1_PTS}pts",
        f"  {cfg.MOMENTUM_ATH_DIST_L3:.0f}-{cfg.MOMENTUM_ATH_DIST_L1:.0f}% → +{cfg.MOMENTUM_ATH_DIST_L3_PTS}pts  |  20-{cfg.MOMENTUM_ATH_DIST_L3:.0f}% → {cfg.MOMENTUM_ATH_DIST_P2_PTS}pts",
        f"  10-20% → {cfg.MOMENTUM_ATH_DIST_P3_PTS}pts  |  &lt;{cfg.MOMENTUM_ATH_DIST_HARD_BLOCK:.0f}% → blocked",
        "",
        "<b>Golden Cross 🌟</b>",
        f"  1H: {cfg.MOMENTUM_GC_1H_MIN:.1f}–{cfg.MOMENTUM_GC_1H_MAX:.0f}%  |  RSI max: {cfg.MOMENTUM_GC_RSI_MAX:.0f}  |  min score: {cfg.MOMENTUM_TOTAL_WATCH}",
        f"  SL {cfg.MOMENTUM_GC_SL_PCT:.0f}%  TP1 {cfg.MOMENTUM_TP1_PCT:.0f}%  TP2 {cfg.MOMENTUM_TP2_PCT:.0f}%",
        "",
        "<b>Volume Spike 📊</b>",
        f"  1H: {cfg.MOMENTUM_VS_1H_MIN:.0f}–{cfg.MOMENTUM_VS_1H_MAX:.0f}%  |  15m vol ≥{cfg.MOMENTUM_VS_VOL_MULT:.0f}× prev-3  |  RSI max: {cfg.MOMENTUM_VS_RSI_MAX:.0f}",
        f"  min score: {cfg.MOMENTUM_TOTAL_WATCH}  |  SL {cfg.MOMENTUM_VS_SL_PCT:.0f}%  TP1 {cfg.MOMENTUM_TP1_PCT:.0f}%  TP2 {cfg.MOMENTUM_TP2_PCT:.0f}%",
        "",
        "<b>Recovery Bounce ♻️</b>",
        f"  1H: {cfg.MOMENTUM_RB_1H_MIN:.0f}–{cfg.MOMENTUM_RB_1H_MAX:.0f}%  |  24H peak ≥{cfg.MOMENTUM_RB_PEAK_PCT:.0f}% above  |  pullback ≥{cfg.MOMENTUM_RB_PULLBACK_PCT:.0f}%",
        f"  4H KDJ J &lt;{cfg.MOMENTUM_RB_KDJ_MAX:.0f}  |  min score: {cfg.MOMENTUM_TOTAL_WATCH}",
        f"  SL {cfg.MOMENTUM_RB_SL_PCT:.0f}%  TP1 {cfg.MOMENTUM_RB_TP1_PCT:.0f}%  TP2 {cfg.MOMENTUM_RB_TP2_PCT:.0f}%",
        "",
        "<b>Pre-Breakout 🔍</b>",
        f"  1H: {cfg.MOMENTUM_PBW_1H_MIN:.0f}–{cfg.MOMENTUM_PBW_1H_MAX:.0f}%  |  5m EMA spread &lt;{cfg.MOMENTUM_PBW_EMA_SPREAD_MAX:.2f}% (compressed)",
        f"  5m RSI streak &lt;{cfg.MOMENTUM_PBW_RSI_MAX:.0f} × {cfg.MOMENTUM_PBW_RSI_CANDLES} candles  |  trigger vol ≥{cfg.MOMENTUM_PBW_VOL_MULT:.1f}× MA10",
        f"  min score: {cfg.MOMENTUM_PBW_MIN_SCORE}  |  SL {cfg.MOMENTUM_PBW_SL_PCT:.0f}%  TP1 {cfg.MOMENTUM_PBW_TP1_PCT:.0f}%  TP2 {cfg.MOMENTUM_PBW_TP2_PCT:.0f}%",
        "",
        "<b>Staircase 🪜</b>",
        f"  1H: {cfg.MOMENTUM_SC_1H_MIN:.0f}–{cfg.MOMENTUM_SC_1H_MAX:.0f}%  |  24H: ≥{cfg.MOMENTUM_SC_PRIOR_MOVE_MIN:.0f}% prior leg  |  15m vol &lt;{cfg.MOMENTUM_SC_VOL_MAX*100:.0f}% MA10",
        f"  15m RSI &lt;{cfg.MOMENTUM_SC_RSI_MAX:.0f} OR KDJ J &lt;{cfg.MOMENTUM_SC_KDJ_MAX:.0f}  |  min score: {cfg.MOMENTUM_SC_MIN_SCORE}",
        f"  SL {cfg.MOMENTUM_SC_SL_PCT:.0f}%  TP1 {cfg.MOMENTUM_SC_TP1_PCT:.0f}%  TP2 {cfg.MOMENTUM_SC_TP2_PCT:.0f}%",
        "",
        "<b>Squeeze Breakout 💥</b>",
        f"  4H EMA spread &lt;{cfg.MOMENTUM_SQUEEZE_EMA_SPREAD_MAX:.0f}%  |  slope &lt;{cfg.MOMENTUM_SQUEEZE_EMA_SLOPE_MAX:.1f}% (flat)",
        f"  15m vol ≥{cfg.MOMENTUM_SQUEEZE_VOL_MULT:.0f}× prev-3  |  RSI {cfg.MOMENTUM_SQ_15M_RSI_MIN:.0f}–{cfg.MOMENTUM_SQ_15M_RSI_MAX:.0f}",
        f"  min score: {cfg.MOMENTUM_SQ_MIN_SCORE}  |  CD: {cfg.MOMENTUM_SQ_COOLDOWN_MIN // 60}H",
        f"  SL {cfg.MOMENTUM_SQ_SL_PCT:.0f}%  TP1 {cfg.MOMENTUM_SQ_TP1_PCT:.0f}%  TP2 {cfg.MOMENTUM_SQ_TP2_PCT:.0f}%",
        "",
        "<b>Speed Alert ⚡</b>",
        f"  15m candle ≥{cfg.MOMENTUM_SPEED_15M_GAIN_MIN:.0f}%  |  vol ≥{cfg.MOMENTUM_SPEED_VOL_MULT:.0f}× prev-3  |  pre-RSI ≤{cfg.MOMENTUM_SPEED_PRE_RSI_MAX:.0f}",
        f"  MCap ≥${cfg.MOMENTUM_SPEED_MCAP_MIN/1e6:.0f}M  |  ATL buffer ≥{cfg.MOMENTUM_SPEED_ATL_BUFFER_PCT:.0f}%",
        f"  SL {cfg.MOMENTUM_SPEED_SL_PCT:.0f}%  TP1 {cfg.MOMENTUM_SPEED_TP1_PCT:.0f}%  TP2 {cfg.MOMENTUM_SPEED_TP2_PCT:.0f}%",
        "",
        "<b>Early GC 🌱</b>",
        f"  1H: {cfg.MOMENTUM_EARLY_GC_1H_MIN:.1f}–{cfg.MOMENTUM_EARLY_GC_1H_MAX:.0f}%  |  5m RSI {cfg.MOMENTUM_EARLY_GC_RSI_MIN:.0f}–{cfg.MOMENTUM_EARLY_GC_RSI_MAX:.0f}",
        f"  5m vol ≥{cfg.MOMENTUM_EARLY_GC_VOL_MIN:.1f}× MA10  |  min score: {cfg.MOMENTUM_EARLY_GC_MIN_SCORE}  |  CD: {cfg.MOMENTUM_EARLY_GC_COOLDOWN_H:.0f}H",
